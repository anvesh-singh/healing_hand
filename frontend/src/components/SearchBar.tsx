...
export const SearchBar = () => {
    const [search,Setsearch]=useState("661c02f51cbcf6e7d87ec6d1");
    const[doctors,Setdoctors]=useState([]);
    const backendcall=async ()=>{
      console.log(search);
        const res= await axios.post(`${BACKEND_URL}/doctor/getall`
         ,{
          "id":search
    })
         Setdoctors(res.data);
        ...
        <div className="max-w-xl">
          ...
          <div className="flex space-x-4">
            <div className="flex rounded-md overflow-hidden w-full">
              ...
              <button  onClick={backendcall} className="bg-indigo-600 text-white px-6 text-lg font-semibold py-4 rounded-r-md">Go</button>
            </div>
          </div>
